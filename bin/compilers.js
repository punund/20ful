// Generated by LiveScript 1.6.0
(function(){
  var Config, fs, log, jstransformer, sass, render, filters, md, formats, compile, out$ = typeof exports != 'undefined' && exports || this;
  Config = require('./config');
  fs = require('fs');
  require('ansicolor').nice;
  log = require('ololog').configure({
    tag: true,
    locate: false
  });
  jstransformer = require('jstransformer');
  sass = require('sass');
  render = function(compiler, text, options, locals){
    options == null && (options = {});
    locals == null && (locals = {});
    return Promise.resolve().then(function(){
      return jstransformer(require("jstransformer-" + compiler)).renderAsync(text, options, locals);
    }).then(function(it){
      return it.body;
    })['catch'](function(e){
      log.error(e);
      log(("Do you have jstransformer-" + compiler + " installed?").lightGreen);
      return process.exit(1);
    });
  };
  filters = {
    markdown: function(){
      return md.render(arguments[0]);
    }
  };
  md = require('markdown-it')({
    html: true
  });
  compose$(toPairs, forEach(function(it){
    var e;
    if (it[1]) {
      try {
        return md.use(require(resolve(it[0])), it[1]);
      } catch (e$) {
        e = e$;
        log.error('Missing module.');
        log('Install', it[0].lightGreen);
        return process.exit(1);
      }
    }
  }))(
  Config['markdown-it-plugins']);
  function resolve(module){
    var local;
    local = process.cwd() + '/node_modules/' + module;
    if (fs.existsSync(local)) {
      return local;
    } else {
      return module;
    }
  }
  formats = {
    js: {
      ls: function(){
        return render('livescript', arguments[0], (import$({
          filename: arguments[1]
        }, arguments[2])));
      }
    },
    css: {
      styl: function(){
        return render('stylus', arguments[0], {
          filename: arguments[1]
        });
      },
      sass: function(){
        return function(it){
          return it.css;
        }(
        sass.compileString(arguments[0], {
          syntax: 'indented'
        }));
      },
      scss: function(){
        return function(it){
          return it.css;
        }(
        sass.compileString(arguments[0]));
      }
    },
    html: {
      md: function(){
        return md.render(arguments[0]);
      },
      pug: function(){
        return render('pug', arguments[0], (import$({
          filters: filters,
          filename: arguments[1]
        }, arguments[2])), arguments[3]);
      },
      njk: function(){
        return render('nunjucks', arguments[0], (import$({
          filters: filters,
          filename: arguments[1]
        }, arguments[2])), arguments[3]);
      }
    }
  };
  compile = function(dst, src, text, filename, options, locals){
    if (dst === src || !dst) {
      return Promise.resolve(text);
    }
    return Promise.resolve(formats[dst][src](text, filename, options, locals));
  };
  out$.formats = formats;
  out$.compile = compile;
  out$.resolve = resolve;
  function compose$() {
    var functions = arguments;
    return function() {
      var i, result;
      result = functions[0].apply(this, arguments);
      for (i = 1; i < functions.length; ++i) {
        result = functions[i](result);
      }
      return result;
    };
  }
  function import$(obj, src){
    var own = {}.hasOwnProperty;
    for (var key in src) if (own.call(src, key)) obj[key] = src[key];
    return obj;
  }
}).call(this);
