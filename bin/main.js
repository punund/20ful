...
(function(){
  var C, Compilers, Toc, fs, Path, mkdirp, glob, stringHash, base58, isbinaryfile, bs, chokidar, frontMatter, pug, args, ref$, state, emptyLayout, site, watcher, allIn, writeOne, processFile, pass2, theError, slice$ = [].slice, join$ = [].join;
  ...
  processFile = function(hsh){
    ...
    return fs.readFile(x.infile).then(function(it){
      var src, dst, fm, body, attr, bust, pj, outfile, link, tocEntry, affected;
      src = tail(ext);
      dst = defaultTo(src)(
      prop('0')(
      ...
      x.link = link;
      x.src = src;
      return Promise.resolve(x.attr.template
        ? (x.done = 1.5, log('template'.red, x.infile.blue), affected = map(function(y){
          return site[y[0]].done = 2;
        })(
        filter(pathEq(['1', 'layout'], x.attr.template))(
        toPairs(
        site))))
        : Compilers.compile(dst, src, body, outfile, attr.options).then(function(compiled){
          switch (false) {
          case !(x.dst === 'html' && x.attr.layout !== 'none'):
            return x.cpld = compiled, x.done = 2, x.layout = x.attr.layout || 'system', x;
          default:
        ...
        if (allIn() && all(compose$(function(it){
          ...
          return Promise.all(pass2()).then(function(){
  ...
  watcher.on('ready', function(){
    if (state.fileCount === 0) {
      log.error("“" + C.source + "/” is empty or doesn't exit. Create it and populate.");
      process.exit(1);
    } else {
